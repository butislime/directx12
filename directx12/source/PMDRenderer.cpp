...
void PMDRenderer::Init(ms::ComPtr<ID3D12Device> device, std::shared_ptr<PMDActor> actor)
{
	this->actor = actor;

	const auto& pmd = actor->GetPMD();
	const auto& vmd = actor->GetVMD();
	// 頂点バッファの作成
	...
	// material
	const auto& materials = actor->GetMaterials();
