...
bool Application::Init()
{
	...
	for (int i = 0; i < pmd.materials.size(); ++i)
	{
		...
		toonResources[i] = LoadTextureFromFile(device.Get(), toonFilePath);
		...
		if (!texFileName.empty())
		{
			...
			textureResources[i] = LoadTextureFromFile(device.Get(), texFilePath);
		}
		...
		if (!sphFileName.empty())
		{
			...
			sphResources[i] = LoadTextureFromFile(device.Get(), sphFilePath);
		}
		...
		if (!spaFileName.empty())
		{
			...
			spaResources[i] = LoadTextureFromFile(device.Get(), spaFilePath);
		}
